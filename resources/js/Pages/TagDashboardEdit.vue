...
<AdminLayout :user="user">
    <div class="w-full">
        <Link :href="route('tags.index')" as="button" class="text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-5">Back</Link>
        <form class="w-full" method="post" @submit.prevent="submit(tag.id)">
            <div class="relative z-0 w-full mb-6 group">
                <input v-model="form.name" type="text" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required="" :disabled="action=='show'">
...
export default {
    ...
    methods: {
        submit(id) {
            this.$inertia.patch(route('tags.update',id), this.form)
        },
    },
}
</script>
