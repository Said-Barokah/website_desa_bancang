...
class UserAccountSettingController extends Controller
{
    public function update(Request $request)
    {
        ...
        if ($validate->fails()) {
            return Redirect::back()->with(['message' => "error untuk diupdate (cek password yang dimasukkan)", "action" => 'error']);
        }
        $auth = Auth::user();
    ...
    public function updatePassword(Request $request)
    {
        ...
        if (password_verify($request->current_password, $auth->password)) {
            ...
            $auth->save();
            return Redirect::back()->with(['message' => "password berhasil di update", "action" => 'success-password']);
        } else {
            return Redirect::back()->with(['message' => "error untuk diupdate (cek password yang dimasukkan)", "action" => 'error']);
        }
    }
